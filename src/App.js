  ...
  const vec = new Vector3(0,0,0)
    ...
    useFrame(() => {
     if(camera.current){
      ...
      if (vehiclepos.x.toFixed(0)<=-50 && vehiclepos.x.toFixed(0)>=-155 && vehiclepos.z<=-60 && vehiclepos.z>=-110){
        camera.current.position.lerp(vec.set(-vehiclepos.x.toFixed(0),-vehiclepos.y.toFixed(0)-24.5,-vehiclepos.z.toFixed(0)-28),0.03)
        camera.current.updateProjectionMatrix();
      }else{
